...
from mng.core.config import INSTANCE_DIR, SERVER_ROOT, log
from mng.routers.auth import require_auth
from mng.utils.docker import is_instance_status, is_instance_restart
...
def get_config(name: str, user=Depends(require_auth)):
    ...
    log.debug(f"[instances] config text={text}")
...
def update_config(name: str, req: ConfigUpdateRequest, user=Depends(require_auth)):
    ...
    log.info(f"[Config] config save={name}")
